...
Sentence Splitting:
Each paragraph is split into up to three sentences. If there are fewer, padding is applied; if more, we truncate after the third sentence. This ensures consistent input size.

Sentence Encoding with RoBERTEctor:
Each sentence is processed independently using RoBERTEctor, a fine-tuned version of RoBERTa tailored for our task. This module generates contextual embeddings that capture the meaning of each sentence.

Multi-Attention Layer:
The sentence embeddings are passed to a multi-attention layer that weighs the importance of each sentence. This allows the model to focus on the most informative parts of the paragraph, rather than treating all sentences equally.

Final Decision:
The weighted sentence representations are aggregated into a single vector, which is used to make the final classification through a fully connected layer.

## Architecture

This architecture combines a fine‑tuned sentence encoder (“RoBERTector”) with a lightweight, attention‑based paragraph aggregator to achieve strong AI vs. human text classification.

---

## 1. Sentence‑Level Expertise  
- **Pretrained & Fine‑Tuned**  
  - Each sentence is encoded by RoBERTector, already trained to spot AI‑generated vs. human style at the sentence level.  
- **Subtle Stylistic Cues**  
  - Captures token‑level patterns, phrasing, function‑word frequencies, and other hallmarks of AI or human writing.  
- **Freezing for Stability**  
  - Freezing the encoder preserves these robust features and prevents overfitting on paragraph data.

---

## 2. Cross‑Sentence Interaction via Multi‑Head Self‑Attention  
- **Beyond Simple Averaging**  
  - Sentence embeddings vary in importance—some contain the thesis, topic shifts, or concluding signals.  
- **Learned Importance Weights**  
  - Attention heads score every sentence against every other, highlighting the most diagnostic ones.  
- **Multiple Perspectives**  
  - Different heads can specialize (e.g., one for discourse markers, another for cohesion, etc.).

---

## 3. Macro‑Pattern Aggregation  
- **Local + Global Signals**  
  - Pools the attended sentence embeddings into a single paragraph vector that encodes:  
    1. **Local signals** (sentence‑level AI/human stylistic cues)  
    2. **Global structure** (discourse flow, abrupt transitions, uniformity)  
- **Robust Paragraph Representation**  
  - Captures both micro‑stylistic and macro‑structural features.

---

## 4. Lightweight Classification Head  
- **Minimal Parameters**  
  - A two‑layer MLP with ReLU and dropout maps the paragraph embedding to a binary decision.  
- **Focused Learning**  
  - Only the attention and MLP head learn; the heavy transformer remains frozen.

---

## Why It Works Better Than Alternatives  
1. **Reuses Proven Features**  
   - Leverages a sentence‑level model already adept at AI vs. human discrimination.  
2. **Adaptive Sentence Weighting**  
   - Attention pinpoints the most telling sentences in each paragraph.  
3. **Models Sentence Interactions**  
   - Captures contradictions or over‑consistency—common in AI text.  
4. **Regularization by Freezing**  
   - Prevents catastrophic forgetting and overfitting on limited paragraph data.  

Together, these components yield higher accuracy and generalization than approaches that either ignore inter‑sentence context or train a full transformer from scratch on small paragraph datasets.  
