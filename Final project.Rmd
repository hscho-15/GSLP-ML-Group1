...
```{r setup, include=FALSE,eval=False, warning = FALSE, message = FALSE}
...
# 2. sentence_weight: JSON sum vectors
df <- df %>%
  mutate(sentence_weight = map_dbl(sentence_weight, ~ {
    tryCatch({
      val <- sum(fromJSON(.))  # ✅ 전체 요소 합
      if (is.null(val)) NA_real_ else val
    }, error = function(e) NA_real_)
  }))

# 3. get mean from each sentence and paragraph
heatmap_df <- df %>%
...
# 4. get valid weigths
valid_weights <- heatmap_df$weight[!is.na(heatmap_df$weight)]

# 5. Heatmap
ggplot(heatmap_df, aes(x = sentence_rank, y = paragraph_id, fill = weight)) +
  geom_tile(color = "white", linewidth = 0.5) +
  scale_fill_gradientn(
    colours = c("#0d0887", "#6a00a8", "#b12a90", "#e16462", "#fca636", "#f0f921"),
    limits = c(min(valid_weights), max(valid_weights)),
    na.value = "grey90"
  ) +
  labs(
    title = "Sum of Sentence Weight by Paragraph and Sentence",
    x = "Sentence Rank",
    y = "Paragraph ID",
    fill = "Weight Sum"
  ) +
...
```
![heat map of sentence weights](images/heatmap.png)
